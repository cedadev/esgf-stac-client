...
def test_1_2_single_datetime():
    result = client.search(datetime='3180-01-01T00:00:00.000Z')
...
def test_1_4_upper_bounded_datetime():
    ...
    for item in result.items():
        item_end_datetime = parser.isoparse(item.properties['end_datetime'])
        assert item_end_datetime <= my_datetime
...
def test_1_5_full_time_datetime():
    ...
    for item in result.items():
        item_start_datetime = parser.isoparse(item.properties['start_datetime'])
        item_end_datetime = parser.isoparse(item.properties['end_datetime'])
        assert my_start_datetime <= item_start_datetime and item_end_datetime <= my_end_datetime
...
def test_1_9_1_full_time_datetime():
...
# 3170-3200 and 3180-1580 partially overlapping. I'm not sure if this item should be returned in the search
def test_1_9_2_full_time_datetime():
    result = client.search(instance_id="CMIP6.CMIP.NASA-GISS.GISS-E2-1-H.piControl.r1i1p1f1.Omon.zostoga.gn.v20190410", datetime='3170-01-01/3200-01-01')
    assert result.matched() == 0
...
def test_2_4_upper_bounded_datetime(load_test_data):
    ...
    for asset in assets:
        asset_end_datetime = parser.isoparse(asset.properties['end_datetime'])
        assert asset_end_datetime <= my_datetime
...
def test_2_5_full_time_datetime(load_test_data):
    ...
    for asset in assets:
        asset_start_datetime = parser.isoparse(asset.properties['start_datetime'])
        asset_end_datetime = parser.isoparse(asset.properties['end_datetime'])
        assert my_start_datetime <= asset_start_datetime and asset_end_datetime <= my_end_datetime

def test_2_6_single_datetime(load_test_data):
    item = TEST_ITEM['data']
    result = item(instance_id="CMIP6.CMIP.NASA-GISS.GISS-E2-1-H.piControl.r1i1p1f1.Omon.zostoga.gn.v20190410", datetime='3581-01-01')
    assert result.matched() == 0
